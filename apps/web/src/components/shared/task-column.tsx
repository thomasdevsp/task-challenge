...
interface TaskColumnProps { title: string, status: TaskStatus }
...
export function TaskColumn({ title, status }: TaskColumnProps) {
  ...
  const assigneeIds = useMemo(() => {
    return Array.from(new Set(taskResponse?.data.flatMap(t => t.assigneeIds) || []));
  }, [taskResponse?.data]);
  ...
  return (
    <div className="p-[2rem] flex flex-col gap-[2rem] border-r-1 border-[#3c3d3f]">
      ...
      <h3 className="text-[1.25rem] font-medium text-[#c2c2c7]">
        {title}
      </h3>
      ...
      {taskResponse?.data.map(task => (
        <TaskCard
          key={task.id}
          task={task}
          assignees={task.assigneeIds.map(id => userMap?.get(id)).filter(Boolean)}
        />
      ))}
